...
BlockTemplate.prototype = {
    nextBlob: function(){
        ...
        return cnUtil.convert_blob(this.buffer,3).toString('hex');
    }
};
    ...
    case "get_template":
		...
		jobRefreshError('Error polling getblocktemplate %j', error);
            ...
            if (!currentBlockTemplate || (previous_hash.toString('hex') !== currentBlockTemplate.previous_hash.toString('hex'))){
                log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d (%s,%s)', [result.height, result.difficulty,previous_hash.toString('hex'),(currentBlockTemplate && currentBlockTemplate.previous_hash)?currentBlockTemplate.previous_hash.toString('hex'):'']);
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            if(config.poolServer.fixedDiff.enabled) {
                ...
                if(fixedDiffCharPos != -1) {
					...
					if (isNaN(difficulty))
					{
						sendReply('invalid static difficutly');
						return;
					}
            ...
            var addressPrefixi = cnUtil.address_decode_integrated(new Buffer(login));
            if ((addressPrefix != sumoWalletBase58AddressPrefix) &&( addressPrefixi != sumoWalletBase58SubAddressPrefix)) {
        ...
        case 'submit':
			...
			if (!shareAccepted){
				redisClient.zadd(config.coin + ':workerHashrate', dateNowSeconds, [job.difficulty, miner.login+':'+miner.pass, dateNow,5].join(':'));
                sendReply('Wrong algo, use monero7 miner');
